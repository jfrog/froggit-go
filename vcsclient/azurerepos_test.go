...
func TestAzureRepos_ListCommits(t *testing.T) {
	ctx := context.Background()
	...
	commits, err := client.ListCommits(ctx, "", repo1, branch1, 3)
	require.NoError(t, err)
	assert.Len(t, commits, 3)
	assert.Equal(t, commits[0], CommitInfo{
	...
	assert.Equal(t, commits[1], CommitInfo{
	...
	assert.Equal(t, commits[2], CommitInfo{
